<!-- Yedek: "gezegen küresi + BAŞARI/SEVİYE/… rozetli" eski hero görseli.
     Geri yüklemek: bu dosyanın <div class="hero-illus" …> … </div> kısmını
     hero-illus.php ile değiştir. -->
      <div class="hero-illus" aria-hidden="true">
        <div class="orbit o2"></div>
        <div class="orbit o1"></div>
        <div class="planet"></div>
        <div class="planet-ring"></div>
        <div class="satellite sat-1">
          <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            <path d="M19 5h-2V3H7v2H5c-1.1 0-2 .9-2 2v1c0 2.55 1.92 4.63 4.39 4.94.63 1.5 1.98 2.63 3.61 2.96V19H7v2h10v-2h-4v-3.1c1.63-.33 2.98-1.46 3.61-2.96C19.08 12.63 21 10.55 21 8V7c0-1.1-.9-2-2-2zM5 8V7h2v3.82C5.84 10.4 5 9.3 5 8zm14 0c0 1.3-.84 2.4-2 2.82V7h2v1z"/>
          </svg>
          <span>BAŞARI</span>
        </div>
        <div class="satellite sat-2">
          <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            <rect x="3" y="13" width="4" height="8" rx="1"/>
            <rect x="10" y="8" width="4" height="13" rx="1"/>
            <rect x="17" y="3" width="4" height="18" rx="1"/>
          </svg>
          <span>SEVİYE</span>
        </div>
        <div class="satellite sat-3">
          <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            <circle cx="12" cy="12" r="10" opacity="0.28"/>
            <circle cx="12" cy="12" r="6.8" opacity="0.55"/>
            <circle cx="12" cy="12" r="3.2" opacity="0.9"/>
            <circle cx="12" cy="12" r="0.8"/>
          </svg>
          <span>HEDEF</span>
        </div>
        <div class="satellite sat-4">
          <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            <path d="M12 3 1 9l4 2.18v6L12 21l7-3.82v-6l2-1.09V17h2V9L12 3zm6.82 6L12 12.72 5.18 9 12 5.28 18.82 9zM17 15.99l-5 2.73-5-2.73v-3.72L12 15l5-2.73v3.72z"/>
          </svg>
          <span>DERSLER</span>
        </div>
      </div>
